	...
	socket.on("game_response", function (data) {
		...
		draw_trigger(function () {
			if (response == "elixir") {
				...
				if (data.claim_pack_cleared) {
					ui_log("Claim tab emptied — tab removed.", "#9D9CFF");
					if (character.user) {
						delete character.user[data.claim_pack_cleared];
						if (character.user.claim_packs) delete character.user.claim_packs[data.claim_pack_cleared];
					}
					if (last_rendered_items == data.claim_pack_cleared) render_gold_npc();
				}
			...
			else if (response == "claim_tab_readonly") ui_log("This tab is withdraw-only — take items out, don't store here.", "#E8A84A");
