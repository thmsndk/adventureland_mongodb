...
function is_monaco_focus(event) {
	if (document.querySelector && document.querySelector("#codeui .monaco-editor.focused, #codeui .monaco-editor-host .focused, #al-vscode-workbench .monaco-editor.focused")) return true;
	var t = event && event.target;
	if (
		t &&
		t.closest &&
		(t.closest(".monaco-editor") ||
			t.closest(".monaco-editor-host") ||
			t.closest(".monaco-aria-container") ||
			t.closest("#code-ide-quick-open") ||
			t.closest(".quick-input-widget") ||
			t.closest(".monaco-quick-input-widget") ||
			t.closest("#al-vscode-workbench") ||
			t.closest(".keybindings-editor") ||
			t.closest(".settings-editor"))
	)
		return true;
	...
	if (
		...
		(ae.closest(".monaco-editor") ||
			ae.closest(".monaco-editor-host") ||
			ae.closest("#code-ide-quick-open") ||
			ae.closest(".quick-input-widget") ||
			ae.closest(".monaco-quick-input-widget") ||
			ae.closest("#al-vscode-workbench") ||
			ae.closest(".keybindings-editor") ||
			ae.closest(".settings-editor"))
	)
		return true;
	return false;
}

/** CODE IDE chrome (explorer/tabs/statusbar) — treat as typing so arrows don't move the character. */
function is_code_ui_focus(event) {
	if (is_monaco_focus(event)) return true;
	if (!window.code) return false;
	var t = (event && event.target) || document.activeElement;
	if (t && t.closest && (t.closest("#codeui") || t.closest("#al-vscode-workbench") || t.closest("#al-vscode-park") || t.closest(".quick-input-widget") || t.closest(".monaco-quick-input-widget")))
		return true;
	return false;
}

/** Chat / non-CODE inputs that should still receive keys while CODE is open. */
function is_game_chat_focus(event) {
	var t = (event && event.target) || document.activeElement;
	if (!t) return false;
	if (t.closest) {
		if (t.closest("#codeui") || t.closest("#al-vscode-workbench") || t.closest("#al-vscode-park") || t.closest(".quick-input-widget") || t.closest(".monaco-quick-input-widget")) {
			return false;
		}
		if (t.closest("#chatinput, #chatlog, .chatinput, #topleftcornerinput, .chat-entity, #mainmenu, .modal, .imodal")) return true;
	}
	if ($("input:focus, textarea:focus, select:focus").length > 0 && !$("#codeui input:focus, #codeui textarea:focus, #al-vscode-workbench textarea:focus").length) {
		return true;
	}
	return false;
}
...
function is_typing_focus(event) {
	...
	if (is_code_ui_focus(event)) return true;
	// While CODE is open, keep bare keys out of the game unless chat/modals are focused.
	// (Empty/broken editor panes often leave focus on <body>, which used to walk the character.)
	if (window.code && !is_game_chat_focus(event)) return true;
	return false;
}
	...
	window.addEventListener(
		...
		function (event) {
			...
			if (is_typing_focus(event)) {
				...
				if (event.keyCode == 27 && window.character && !is_monaco_focus(event)) {
					/* fall through */
				} else return;
			}
