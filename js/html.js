...
function is_claim_pack(pack) {
	return !!(character.user && character.user.claim_packs && character.user.claim_packs[pack] && character.user.claim_packs[pack].withdraw_only);
}

function claim_pack_title(pack) {
	if (!is_claim_pack(pack)) return "";
	var meta = character.user.claim_packs[pack];
	return meta.label || "Season Claim";
}
...
function render_items_npc(pack) {
	...
	var claim = is_claim_pack(pack);
	var html = "<div style='background-color: black; border: 5px solid " + (claim ? "#D4A017" : "gray") + "; padding: 2px; font-size: 24px; display: inline-block' class='dcontain'>";
	if (claim) {
		html +=
			"<div style='padding: 6px 8px 4px; font-size: 14px; color: #F5D76E; border-bottom: 1px solid #6B5420; margin-bottom: 4px; text-align: center'>" +
			claim_pack_title(pack) +
			"<br><span style='color: #C8B080; font-size: 12px'>Withdraw-only — take items to your inventory</span></div>";
	}
...
function on_rclick(current) {
	...
	if (on) smart_eval(on);
		...
		if (topleft_npc == "items") {
			if (is_claim_pack(last_rendered_items)) {
				ui_log("This tab is withdraw-only — take items out, don't store here.", "#E8A84A");
				return;
			}
			tut("store");
