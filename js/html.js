...
// Renders active buffs/debuffs for the local player to the HUD without opening the character window
function render_hud_conditions() {
	if (!window.character) return;
	var $container = $("#hudeffects");
	if (!$container.length) return;

	var player = window.character;
	...
	for (var condition in player.s) {
		if (G.skills[condition] && G.skills[condition].ui) {
			activeConditions[condition] = {
				type: "skill",
				def: G.skills[condition],
				ms: player.s[condition].ms,
			};
		} else {
			var prop = G.conditions[condition];
	...
	for (var condition in activeConditions) {
		var condData = activeConditions[condition];
		var $existing = $wrap.find("[data-condition='" + condition + "']");

		if (!$existing.length) {
			var html;
			var isDebuff = !!(condData.type === "condition" && condData.prop && condData.prop.debuff);

			if (condData.type === "skill") {
				var rid = "cond_" + condition;
				html = item_container({
					skin: condData.def.skin,
					loader: "hudc" + rid,
					noBorder: !isDebuff,
					noBackground: true,
				...
				html = item_container(
					{
						skin: condData.actual.skin || (condData.prop && condData.prop.skin),
						onclick: "condition_click('" + condition + "')",
						noBorder: !isDebuff,
						noBackground: true,
						debuffBorder: isDebuff,
					},
			...
			var $newElement = $(html);
			$newElement.attr("data-condition", condition);
			$wrap.append($newElement);

			if (condData.type === "skill") {
				var rid2 = "cond_" + condition;
				$(".loaderhudc" + rid2).css("opacity", 0.5);
				add_tint(".loaderhudc" + rid2, {
					ms: condData.ms,
					start: future_ms(condData.ms - 24000),
					type: "progress",
				});
			}
		}
	...
	$wrap.find("[data-condition]").each(function () {
		var condName = $(this).attr("data-condition");
		if (!activeConditions[condName]) {
			$(this).remove();
		}
	});

	$wrap.find("[data-spacer='true']").remove();
